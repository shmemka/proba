...
import { signIn, signInWithGoogle, getCurrentUser, isSupabaseAvailable } from '@/lib/supabase'
      ...
      <div className="max-w-md w-full space-y-8 sm:space-y-10">
        <div>
          ...
          <p className="text-center text-sm sm:text-base font-light text-primary-600">
            Продолжите с Google или войдите через email
          </p>
        </div>

        {/* Google Sign In Button - основной способ */}
        {isSupabaseAvailable() && (
          <button
            type="button"
        ...
        {/* Переключатель между Google и Email формой */}
        {!showEmailForm ? (
          <div className="text-center">
            <button
              type="button"
              onClick={() => setShowEmailForm(true)}
              className="text-sm font-light text-primary-600 hover:text-primary-900 transition-colors"
            >
          ...
          <>
            ...
            <div className="text-center text-sm">
              <p className="font-light text-primary-600">
                Нет аккаунта?{' '}
                <span className="text-primary-600">используйте email и пароль ниже</span>
              </p>
            </div>
          </>
        )}
