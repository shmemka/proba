  ...
  useEffect(() => {
    const handleOAuthCallback = async () => {
      ...
      try {
        ...
        // Проверяем код авторизации в query параметрах
        ...
        if (code) {
          ...
          // Проверяем сессию напрямую (Supabase может автоматически обработать hash)
        ...
        // Если сессия получена, создаем профиль и перенаправляем
        if (session?.user || user) {
          const currentUser = user || session?.user
          
          if (currentUser) {
            // Создаем профиль специалиста для нового пользователя
            try {
            ...
            setStatus('success')
            // Перенаправляем на главную страницу специалистов
            setTimeout(() => {
          ...
          // Ждем немного и проверяем снова (на случай, если Supabase обрабатывает hash асинхронно)
          setTimeout(async () => {
