...
-- ============================================
-- Убираем таблицу companies, привязываем projects напрямую к auth.users

-- 1. Удаляем старую таблицу companies (если есть)
DROP TABLE IF EXISTS applications CASCADE;
DROP TABLE IF EXISTS projects CASCADE;
DROP TABLE IF EXISTS companies CASCADE;
-- specialists оставляем
...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'specialists') THEN
    ...
    CREATE POLICY "Пользователи могут создавать свой профиль специалиста"
...
-- Триггеры для specialists (если еще не создан)
DO $$
BEGIN
...
-- ============================================
-- Структура упрощена:
-- - Убрана таблица companies
-- - projects привязаны напрямую к auth.users через user_id
