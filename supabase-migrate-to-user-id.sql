...
-- 2. Копируем данные из company_id в user_id (если company_id существует)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 3. Устанавливаем user_id для всех проектов, где он NULL (берем из auth.users)
DO $$
DECLARE
...
-- 4. Удаляем старые политики, которые зависят от company_id (ПЕРЕД удалением колонки!)
...
-- 5. Удаляем foreign key constraint для company_id
ALTER TABLE projects DROP CONSTRAINT IF EXISTS projects_company_id_fkey;

-- 6. Удаляем старую колонку company_id (теперь политики уже удалены)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 7. Устанавливаем NOT NULL для user_id (только если все проекты имеют user_id)
DO $$
BEGIN
...
-- 8. Добавляем foreign key constraint для user_id
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 9. Создаем индекс для user_id (если его нет)
CREATE INDEX IF NOT EXISTS idx_projects_user_id ON projects(user_id);

-- 10. Создаем новые политики для user_id (старые уже удалены в шаге 4)
DO $$
BEGIN
...
-- 11. Обновляем политики для applications (используем user_id вместо company_id)
...
-- 12. Удаляем таблицу companies (ОСТОРОЖНО: это удалит все данные компаний!)
